...
[ServiceNow Basic Auth or Token Connection Configuration](https://github.com/shadowman-lab/Ansible-SNOW/tree/main/SNOWSetup#servicenowaap-integration-instructions-using-event-driven-ansible-and-snow-business-rules)
...
- This has been tested with:
  - Ansible Tower 3.6, 3.7, 3.8, AAP 2.0, 2.1, 2.2, 2.3, 2.4, 2.5, 2.6, 2.7
...
## Notes
Why Async? "After" rules run in the same database transaction. If the transaction hasn't "committed" yet, the relationship between the RITM and the REQ might not exist in the database. Async runs just after the transaction finishes, meaning the REQ is guaranteed to be there.

## Note
The first line in the script below must match the Rest Message name and the HTTP Method name that was created earlier
...
(function executeRule(current, previous) {
  ...
  var reqId = current.getValue('request');
  if (reqId) {
    var grReq = new GlideRecord('sc_request');
    if (grReq.get(reqId)) {
        json["request_number"] = grReq.getValue('number'); // This gets REQ001234
    }
  } else {
    // If it's STILL empty, we check the Parent field (sometimes used instead of Request)
    json["request_number"] = current.parent ? current.parent.getDisplayValue() : "NOT_FOUND";
  }
